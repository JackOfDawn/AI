...
void Sprite::draw(GraphicsBuffer& dest, float dx, float dy, float rotationInRadians, float scale, ALLEGRO_COLOR color, int flags)
{
	if( mpBitmap != NULL && dest.getBitmap() != NULL )
	{
		...
		if( rotationInRadians != 0.0f )//need to rotate
		{
			...
			//al_draw_scaled_rotated_bitmap(mpBitmap, centerX, centerY, dx, dy, scale, scale, rotationInRadians, flags);
			al_draw_tinted_scaled_rotated_bitmap(mpBitmap, color, centerX, centerY, dx, dy, scale, scale, rotationInRadians, flags);
		}
		else//no need to rotate
		{
			//al_draw_bitmap( mpBitmap, dx, dy, flags );
			al_draw_scaled_bitmap(mpBitmap, 0, 0, mWidth, mHeight, dx, dy, mWidth * scale, mHeight * scale, flags);
		}
		//restore old target
