...
void Sprite::drawRect(GraphicsBuffer& dest, float dx, float dy, float width, float height, int flags)
{
	if (mpBitmap != NULL && dest.getBitmap() != NULL)
	{
		//set new target
		ALLEGRO_BITMAP* pOldTarget = GraphicsSystem::switchTargetBitmap(dest.getBitmap());
		//draw
		ALLEGRO_COLOR color = al_map_rgb(1, 1, 1);
		al_draw_filled_rectangle(dx, dy, width, height, color);
		//restore old target
		GraphicsSystem::switchTargetBitmap(pOldTarget);
	}
}
