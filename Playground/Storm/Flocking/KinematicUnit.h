...
{
public:
	KinematicUnit(Sprite* pSprite, const Vector2D& position, float orientation, const Vector2D& velocity, float rotationVel, float maxVelocity = 1.0f, float maxAcceleration = 1.0f, float radius = 8);
	~KinematicUnit();
	...
	std::vector<KinematicUnit*> getNeighbors() const { return mNeighbors; }


	virtual void setNewOrientation();//face the direction you are moving

	void handleCylinderCollisions();
	void handleWallCollisions(float time);
	Vector2D handleWallAvoidance();
	...
	void flock(KinematicUnit* pTarget);
